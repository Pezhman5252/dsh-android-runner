...
export function classifyGradleError(stdout, stderr, timedOut = false) {
    if (timedOut)
        return { type: 'TIMEOUT', message: 'Gradle execution exceeded the configured timeout.' };
    const text = `${stdout}\n${stderr}`;
    for (const [type, pattern] of patterns)
        if (pattern.test(text))
            return { type, message: diagnosis(type) };
