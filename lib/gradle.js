...
export async function runGradle(projectRoot, gradleArgs, timeoutMs, signal, useSystemGradle = false) {
    ...
    const child = windows
        ? spawn(useSystemGradle ? (process.env.ComSpec || 'cmd.exe') : (process.env.ComSpec || 'cmd.exe'), useSystemGradle
            ? ['/d', '/s', '/c', 'gradle.bat', ...gradleArgs]
            : ['/d', '/s', '/c', 'gradlew.bat', ...gradleArgs], { cwd: projectRoot, windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'], detached: false })
...
export function systemGradleAvailable() {
...
export function reportRoots(projectRoot, modules) {
    return modules.map((modulePath) => join(gradleProjectPathToDirectory(projectRoot, modulePath), 'build'));
}
export function relativeModuleDirectory(projectRoot, modulePath) {
