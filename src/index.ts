...
interface ToolArgs {
  ...
  parallel?: boolean
  gradleProperties?: Record<string, unknown>
  useSystemGradle?: boolean
  detailedReport?: boolean
  debug?: boolean
  compareWithPrevious?: boolean
  coverage?: boolean
}
interface Comparison { previousAvailable: boolean; failedDelta: number; durationDeltaMs: number }
interface ToolResult {
  success: boolean
  executionStatus: 'passed' | 'failed' | 'no_previous_failures' | 'cancelled' | 'timeout' | 'execution_error' | 'parse_error'
  message: string
  projectRoot: string
  testType: 'jvm' | 'instrumentation'
  gradleTask: string
  selectedFilters: string[]
  durationMs: number
  summary: ReportParseResult
  gradleErrorType: string
  gradleErrorMessage: string
  reportPaths: string[]
...
function sessionCwd(execCtx: any): string {
  ...
  if (typeof cwd !== 'string' || !cwd.trim()) throw new Error('The active DSH session has no workspace/cwd. Start the tool from an active project session.')
  const stat = statSync(cwd)
  if (!stat.isDirectory()) throw new Error('The active DSH session cwd is not a directory.')
  return cwd
}
function validateLimits(args: ToolArgs): { timeoutMs: number; maxFilters: number; maxOutputTail: number } {
  const timeoutMs = args.timeoutMs ?? DEFAULT_TIMEOUT_MS
  if (!Number.isInteger(timeoutMs) || timeoutMs < 1000 || timeoutMs > MAX_TIMEOUT_MS) throw new Error(`timeoutMs must be an integer between 1000 and ${MAX_TIMEOUT_MS}.`)
  const maxFilters = args.maxFilters ?? DEFAULT_MAX_FILTERS
  if (!Number.isInteger(maxFilters) || maxFilters < 1 || maxFilters > HARD_MAX_FILTERS) throw new Error(`maxFilters must be an integer between 1 and ${HARD_MAX_FILTERS}.`)
  const maxOutputTail = args.maxOutputTail ?? DEFAULT_MAX_OUTPUT_TAIL
  if (!Number.isInteger(maxOutputTail) || maxOutputTail < 500 || maxOutputTail > HARD_MAX_OUTPUT_TAIL) throw new Error(`maxOutputTail must be an integer between 500 and ${HARD_MAX_OUTPUT_TAIL}.`)
  return { timeoutMs, maxFilters, maxOutputTail }
}
function outputTail(stdout: string, stderr: string, maxChars: number): string {
  const text = `${stdout}${stderr ? `\n[stderr]\n${stderr}` : ''}`.trim()
  return text.length <= maxChars ? text : text.slice(-maxChars)
}
function uniqueFilters(failures: TestFailure[], maxFilters: number): string[] {
  const result: string[] = []
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args: unknown, value: unknown) => {
        ...
        if (result.coverage.available) lines.push(`- **JaCoCo:** line ${formatPct(result.coverage.linePercent)}, branch ${formatPct(result.coverage.branchPercent)}, instruction ${formatPct(result.coverage.instructionPercent)}`)
        if (result.comparison.previousAvailable) lines.push(`- **Previous run:** failed delta ${result.comparison.failedDelta}, duration delta ${result.comparison.durationDeltaMs} ms`)
        if (result.selectedFilters.length) lines.push(`- **Filters:** ${result.selectedFilters.join(', ')}`)
        if (result.summary.failuresList.length) { lines.push('', '**Failures:**'); for (const f of result.summary.failuresList.slice(0, 20)) lines.push(`- ${f.testClass}#${f.testName}\n  ${f.error.replace(/\s+/g, ' ').slice(0, 1200)}`) }
        if (result.summary.slowestTests.length) { lines.push('', '**Slowest tests:**'); for (const t of result.summary.slowestTests) lines.push(`- ${t.testClass}#${t.testName}: ${t.durationMs} ms`) }
        if (result.reportPaths.length) lines.push('', '**Fresh report files:**', ...result.reportPaths.map((p) => `- ${p}`))
    ...
    async execute(args: ToolArgs, execCtx: any): Promise<ToolResult> {
      ...
      if (args.rerunFailed && args.testFilter) throw new Error('rerunFailed and testFilter cannot be used together.')
      ...
      if (modules.length > MAX_MODULES) throw new Error(`A maximum of ${MAX_MODULES} modules is supported.`)
      if (!wrapperExists(projectRoot) && !args.useSystemGradle) throw new Error(`Gradle wrapper not found. Set useSystemGradle=true only when a trusted system Gradle installation is intentionally available.`)
      ...
      const previousSummaries: ReportParseResult[] = []
      if (args.rerunFailed) {
        if (modules.length > 1) {
      ...
      if (testType === 'jvm') for (const filter of selectedFilters) gradleArgs.push('--tests', filter)
      if (testType === 'instrumentation' && selectedFilters.length === 1) gradleArgs.push(`-Pandroid.testInstrumentationRunnerArguments.class=${selectedFilters[0]}`)
      gradleArgs.push(...properties)
      if (args.parallel && modules.length > 1) gradleArgs.push('--parallel')
      if (args.continueOnFailure) gradleArgs.push('--continue')
      gradleArgs.push('--rerun-tasks', '--no-daemon', '--console=plain')
      if (args.debug) logger.info(`run_robolectric debug: type=${testType} modules=${modules.join(',')} tasks=${tasks.join(',')} filters=${selectedFilters.length}`)
      else logger.info(`Running ${tasks.join(', ')} in ${projectRoot}`)
      ...
      const preRunReports = new Map<string, number>()
      for (const file of new Set(modules.flatMap((module, i) => collectReportFiles(projectRoot, module, taskNames[i]!, testType === 'instrumentation')))) {
        try { preRunReports.set(file, statSync(file).mtimeMs) } catch { /* file vanished between listing and stat */ }
      }

      const startedAt = Date.now()
      let run
      try { run = await runGradle(projectRoot, gradleArgs, limits.timeoutMs, execCtx.signal, Boolean(args.useSystemGradle)) }
      catch (error) {
      ...
      if (run.timedOut) return { success: false, executionStatus: 'timeout', message: `Gradle exceeded the ${limits.timeoutMs} ms timeout.`, projectRoot, testType, gradleTask: tasks.join(' '), selectedFilters, durationMs: run.durationMs, summary: emptySummary(), gradleErrorType: 'TIMEOUT', gradleErrorMessage: 'Gradle execution exceeded the configured timeout.', reportPaths: [], coverage: emptyCoverage(), comparison: emptyComparison(), rawOutputTail }
      if (run.aborted || execCtx.signal?.aborted) return { success: false, executionStatus: 'cancelled', message: 'Gradle execution was cancelled.', projectRoot, testType, gradleTask: tasks.join(' '), selectedFilters, durationMs: run.durationMs, summary: emptySummary(), gradleErrorType: 'UNKNOWN', gradleErrorMessage: 'Execution was cancelled.', reportPaths: [], coverage: emptyCoverage(), comparison: emptyComparison(), rawOutputTail }
      ...
      const freshReportFiles = uniqueReportFiles.filter((file) => {
        try {
          ...
          if (previous !== undefined && mtime <= previous) return false
          return true
        } catch { return false }
      })
      ...
      const currentHistory: HistoryEntry = { timestamp: new Date().toISOString(), task: tasks.join(' '), modules, testType, total: summary.total, passed: summary.passed, failed: summary.failed, skipped: summary.skipped, durationMs: run.durationMs }
      const comparison = args.compareWithPrevious ? compareSummary(loadPrevious(projectRoot), currentHistory) : emptyComparison()
      if (args.compareWithPrevious) saveHistory(projectRoot, currentHistory)
